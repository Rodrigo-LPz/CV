...
export function SiteShell() {
  ...
  const [activeReferenceFilter, setActiveReferenceFilter] = useState('Todos');
  const [openReference, setOpenReference] = useState<string | null>(null);
  const content = useMemo(() => siteContent[locale], [locale]);
  useEffect(() => {
    setActiveReferenceFilter(locale === 'es' ? 'Todos' : 'All');
  }, [locale]);
  const allReferenceTags = useMemo(() => {
    const tags = new Set(content.referencesProjects.flatMap((project) => project.tags));
    return [locale === 'es' ? 'Todos' : 'All', ...Array.from(tags)];
  }, [content.referencesProjects, locale]);

  const filteredReferences = useMemo(() => {
    const isAll = activeReferenceFilter === (locale === 'es' ? 'Todos' : 'All');
    if (isAll) {
      return content.referencesProjects;
    }
    return content.referencesProjects.filter((project) =>
      project.tags.includes(activeReferenceFilter),
    );
  }, [activeReferenceFilter, content.referencesProjects, locale]);
    ...
    <div className="mx-auto max-w-6xl px-6 pb-14 pt-8 md:px-10">
      ...
      <main className="space-y-8">
        ...
        >
          ...
          <h2 className="mb-2 text-2xl font-bold text-white">{content.sections.references.title}</h2>
          <p className="mb-5 text-sm leading-relaxed text-[var(--muted)]">
            {content.sections.references.description}
          </p>

          <div className="mb-5 flex flex-wrap gap-2">
            {allReferenceTags.map((tag) => {
              const active = tag === activeReferenceFilter;
              return (
                <button
                  key={tag}
                  type="button"
                  onClick={() => setActiveReferenceFilter(tag)}
                  className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-semibold transition ${
                    active
                      ? 'border-[var(--accent)] bg-[var(--accent)]/15 text-[var(--accent)]'
                      : 'border-white/15 bg-white/5 text-[var(--muted)] hover:border-[var(--accent)] hover:text-[var(--accent)]'
                  }`}
                >
          ...
          <div className="grid gap-3">
            {filteredReferences.map((project) => {
              const expanded = openReference === project.title;
              return (
                <article
                  key={project.title}
                  className="rounded-2xl border border-white/10 bg-[var(--surface)]/45"
                >
                  <button
                    type="button"
                    onClick={() => setOpenReference((prev) => (prev === project.title ? null : project.title))}
                  ...
                  >
                    ...
                    <ChevronDown
                      size={18}
                      className={`shrink-0 text-[var(--accent)] transition ${expanded ? 'rotate-180' : ''}`}
                    />
                  </button>
                      ...
                      <div className="mb-3 flex flex-wrap gap-2">
                        {project.tags.map((tag) => (
                          <span
                            ...
                            className="rounded-full border border-white/15 bg-white/5 px-2.5 py-1 text-xs text-[var(--muted)]"
                          >
                            {tag}
                          </span>
                        ))}
                      ...
                      <div className="flex flex-wrap gap-2 text-xs font-semibold">
                        {project.demo && (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-1 rounded-full border border-white/20 px-3 py-1.5 text-[var(--muted)] hover:border-[var(--accent)] hover:text-[var(--accent)]"
                          >
